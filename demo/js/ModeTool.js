...
ModeTool.prototype = {
	...
	createBaseMode : function (top, left, imgSrc, modeIndex, backWidth, backHeight, isText, isButton) {
		...
		var textNode = doc.createTextNode("请输入文本");
		...
		if(isText) {
			...
			// button 文字放在content中
			titleDiv.appendChild(buttonNode);
			contentDiv.appendChild(titleDiv);
			contentDiv.appendChild(backImg);
			moduleDiv.appendChild(contentDiv);
			...
			self.setClass(moduleDiv, "module");
			backImg.style.width = backWidth;
			// module 文字放在moduleDiv中，与content平级
	...
	create : function (top, left, imgSrc, isText, isButton) {
	
		var self = this,
		modeIndex = self.getNextIndex(),
		doc = document,

		moduleDiv = self.createBaseMode(top, left, imgSrc, modeIndex, "50px", "50px", isText, isButton);
		...
		// 20170117新增 新创建module后其他module选中状态消失
	...
	drag : function (mode) {
		// mode 是content， parentMode 才是 moduleDiv
		var parentMode = mode.parentNode,

		// 查看content里是否有titleDiv
	...
	// mode是contentDiv
      ...
      var x = event.pageX - parentMode.getBoundingClientRect().left;
          y = event.pageY - parentMode.getBoundingClientRect().top;
	...
	dragPoint : function (point) {
		...
		point.onmousedown = function(event) {
			...
			var x = event.layerX && event.layerX >= 0 ? event.layerX : event.offsetX,
					y = event.layerY && event.layerY >= 0 ? event.layerY : event.offsetY;
	...
	getSonNode : function (mode, nodeClassName) {
		...
		for (var copyNode = mode.firstChild; copyNode != null; copyNode = copyNode.nextSibling) {
			...
			if (copyNode.nodeType == 1) {
				...
				if ((className == "content" || className == "button-content" || className == "text-content") && (nodeClassName == "backImg" || nodeClassName == "title")) {
				
					for(var sonNode = copyNode.firstChild; sonNode != null; sonNode = sonNode.nextSibling) {
					
						if (sonNode.nodeType == 1) {
							
							if (nodeClassName == "backImg" && sonNode.src) {
								return sonNode;
							} else if (nodeClassName == "title") {
								return sonNode;
							}
	...
	setIndex : function (mode, idIndex, oldIndex) {
		...
		for (var copyNode = mode.firstChild; copyNode != null; copyNode = copyNode.nextSibling) {
			...
			if (copyNode.nodeType == 1) {
				...
				var className = copyNode.className;
				copyNode.id = copyNode.id.replace(oldIndex, idIndex);
			
				if (className == "content" || className == "text-content" || className == "button-content") {
				
					for(var sonNode = copyNode.firstChild; sonNode != null; sonNode = sonNode.nextSibling) {
					
						if (sonNode.nodeType == 1) {
							sonNode.id = sonNode.id.replace(oldIndex, idIndex);
						}
	...
	copy : function (mode) {
		...
		oldIndex = self.getModeIndex(mode),
		idIndex = self.getNextIndex();
	
		self.setIndex(copyObj, idIndex, oldIndex);
	  ...
	  var global = com.xjwgraph.Global;
	  var modeMap = global.modeMap;
	  
		var baseMode = new BaseMode();
		baseMode.id = copyObj.id;
		baseMode.modeType = modeMap.get(mode.id).modeType;
	  
		modeMap.put(baseMode.id, baseMode);
